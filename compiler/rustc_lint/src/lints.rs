...
impl<'a> LintDiagnostic<'a, ()> for BuiltinTypeAliasBounds<'_> {
    fn decorate_lint<'b>(self, diag: &'b mut Diag<'a, ()>) {
        ...
        diag.note(msg!("this is a known limitation of the type checker that may be lifted in a future edition.
        ...
        match self {
            ...
            NonLocalDefinitionsDiag::MacroRules {
                ...
                if doctest {
                    ...
                    diag.help(msg!("remove the `#[macro_export]` or move this `macro_rules!` outside the of the current {$body_kind_descr} {$depth ->
                        [one] `{$body_name}`
    ...
    fn add_to_diag<G: EmissionGuarantee>(self, diag: &mut Diag<'_, G>) {
        match self {
            OverflowingBinHexSign::Positive => {
                diag.note(msg!("the literal `{$lit}` (decimal `{$dec}`) does not fit into the type `{$ty}` and will become `{$actually}{$ty}`"));
            }
            OverflowingBinHexSign::Negative => {
                diag.note(msg!(
                    "the literal `{$lit}` (decimal `{$dec}`) does not fit into the type `{$ty}`"
                ));
                diag.note(msg!("and the value `-{$lit}` will become `{$actually}{$ty}`"));
            }
        }
    }
}
